...
| domain | String | `window.location.hostname` | Override the `domain` value reported in event metadata. Useful for multi-tenant apps where many subdomains belong to one app monitor and events should be attributed to a single canonical domain. When unset, falls back to `window.location.hostname` (the historical default). |
